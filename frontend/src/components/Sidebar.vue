...
<template>
  ...
  <aside
    class="hidden sm:flex flex-col bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 sticky top-0 h-screen overflow-y-auto transition-all duration-300"
  ...
  >
    ...
    <nav class="flex-1 space-y-2 py-4 px-2 sm:px-1">
      ...
      <router-link
        ...
        :class="{ 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400': isActive('home') }"
      >
        <svg class="w-6 h-6 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
          <path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/>
        </svg>
        <span v-if="isExpanded" class="hidden lg:inline font-semibold text-base">Anasayfa</span>
      </router-link>
      ...
      <router-link
        to="/explore"
        class="flex items-center justify-center lg:justify-start gap-4 px-4 py-3 rounded-full text-gray-700 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-600 dark:hover:text-primary-400 transition-all duration-200"
        :class="{ 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400': isActive('explore') }"
      >
        <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
        </svg>
        <span v-if="isExpanded" class="hidden lg:inline font-semibold text-base">Keşfet</span>
      </router-link>
      ...
      <router-link
        ...
        :class="{ 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400': isActive('notifications') }"
      >
        ...
        <span v-if="isExpanded" class="hidden lg:inline font-semibold text-base">Bildirimler</span>
      </router-link>
      ...
      <router-link
        to="/messages"
        class="relative flex items-center justify-center lg:justify-start gap-4 px-4 py-3 rounded-full text-gray-700 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-600 dark:hover:text-primary-400 transition-all duration-200"
        :class="{ 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400': isActive('messages') }"
      >
        <div class="relative flex-shrink-0">
        ...
        <span v-if="isExpanded" class="hidden lg:inline font-semibold text-base">Sohbet</span>
      </router-link>
      ...
      <router-link
        :to="`/profile/${userId}`"
        class="flex items-center justify-center lg:justify-start gap-4 px-4 py-3 rounded-full text-gray-700 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-600 dark:hover:text-primary-400 transition-all duration-200"
        :class="{ 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400': isActive('profile') }"
      >
        <svg class="w-6 h-6 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 12a4 4 0 100-8 4 4 0 000 8zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/>
        </svg>
        <span v-if="isExpanded" class="hidden lg:inline font-semibold text-base">Profil</span>
      </router-link>

      <!-- Ayarlar -->
      <button
        @click="showSettings = !showSettings"
        class="w-full flex items-center justify-center lg:justify-start gap-4 px-4 py-3 rounded-full text-gray-700 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-600 dark:hover:text-primary-400 transition-all duration-200"
        :class="{ 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400': showSettings }"
      >
        <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <span v-if="isExpanded" class="hidden lg:inline font-semibold text-base">Ayarlar</span>
      </button>
    </nav>

    <!-- MAKULE Button (Post) -->
    <div class="px-2 sm:px-1 py-4 border-t border-gray-200 dark:border-gray-700">
      <button
        @click="createPost"
