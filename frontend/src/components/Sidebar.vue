...
<template>
  ...
  >
    <!-- Logo Section -->
    <button
      @click="handleNavClick({ id: 'home', path: '/' })"
      class="flex items-center gap-4 pt-4 pb-2 px-4 flex-shrink-0 outline-none"
    ...
    <nav class="flex-1 space-y-1 py-2 px-2 sm:px-1">
      <button
        ...
        class="group flex items-center justify-center lg:justify-start gap-4 px-4 py-3 rounded-full text-slate-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-primary-900/20 hover:text-blue-600 dark:hover:text-primary-400 transition-all duration-200 w-full outline-none"
    ...
    <div class="relative px-2 py-4 border-t border-slate-100 dark:border-gray-800">
      <div v-if="showUserMenu" class="absolute bottom-full left-0 w-full mb-2 bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-2xl shadow-2xl overflow-hidden z-[60] animate-in fade-in slide-in-from-bottom-2 duration-200">
        <button v-if="authStore.user?.role === 'ADMIN' || authStore.user?.email === '2312101063@ogr.mehmetakif.edu.tr'" @click="goToAdmin" class="w-full flex items-center gap-3 px-4 py-3 text-sm font-black text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors border-b border-slate-100 dark:border-gray-800">
        ...
        <button v-if="authStore.user?.role === 'ACADEMIC'" @click="goToAcademic" class="w-full flex items-center gap-3 px-4 py-3 text-sm font-black text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors border-b border-slate-100 dark:border-gray-800">
          <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
          <span>Akademisyen Paneli</span>
        </button>
        <button @click="goToProfile" class="w-full flex items-center gap-3 px-4 py-3 text-sm font-semibold text-slate-700 dark:text-gray-300 hover:bg-slate-50 dark:hover:bg-gray-800 transition-colors border-b border-slate-50 dark:border-gray-800">
          <svg class="w-5 h-5 text-slate-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
          <span class="text-slate-700 dark:text-gray-200">Profili Gör</span>
        </button>
        <button @click="goToSettings" class="w-full flex items-center gap-3 px-4 py-3 text-sm font-semibold text-slate-700 dark:text-gray-300 hover:bg-slate-50 dark:hover:bg-gray-800 transition-colors border-b border-slate-50 dark:border-gray-800">
        ...
        <button @click="handleLogout" class="w-full flex items-center gap-3 px-4 py-3 text-sm font-semibold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors">
          <svg class="w-5 h-5 text-red-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
          <span>Çıkış Yap</span>
        </button>
      </div>

      <button @click.stop="showUserMenu = !showUserMenu" class="w-full flex items-center gap-3 p-3 rounded-full hover:bg-slate-100 dark:hover:bg-gray-800 transition-all duration-200 group relative z-[70]" :class="{ 'bg-slate-100 dark:bg-gray-800': showUserMenu }">
        <div class="relative flex-shrink-0">
          <div class="p-[2px] rounded-full bg-gradient-to-tr from-blue-400 to-purple-400 shadow-sm group-hover:scale-105 transition-transform">
            <img v-if="authStore.user?.avatarUrl" :src="authStore.user.avatarUrl" class="w-10 h-10 rounded-full object-cover border-2 border-white dark:border-gray-950" />
            <div v-else class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 border-2 border-white dark:border-gray-950 flex items-center justify-center text-white font-black text-sm">{{ authStore.user?.username?.charAt(0).toUpperCase() }}</div>
          </div>
        </div>
        <div v-if="isExpanded" class="flex-1 min-w-0 text-left">
          <p class="text-sm font-black text-slate-900 dark:text-white truncate">{{ authStore.user?.fullName || authStore.user?.username }}</p>
          <p class="text-xs text-slate-500 dark:text-gray-400 truncate">@{{ authStore.user?.username }}</p>
        </div>
        <svg v-if="isExpanded" class="w-4 h-4 text-slate-400 transition-transform duration-200" :class="{ 'rotate-180': showUserMenu }" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
      </button>
    </div>
  </aside>
...
const filteredNavItems = computed(() => navItems.filter(item => item.id !== 'requests' || authStore.user?.isPrivate));
...
const goToProfile = () => { showUserMenu.value = false; router.push(`/profile/${authStore.user?.username}`); };
const goToSettings = () => { showUserMenu.value = false; router.push('/settings'); };
const goToAdmin = () => { showUserMenu.value = false; router.push('/admin'); };
const goToAcademic = () => { showUserMenu.value = false; router.push('/academic'); };
const handleLogout = () => { showUserMenu.value = false; chatStore.resetStore(); authStore.logout(); router.push("/auth"); toast.success("Çıkış yapıldı."); };

const fetchUnreadCounts = async () => {
  try {
    await notificationsStore.fetchUnreadCount();
    await chatStore.fetchConversations();
    if (authStore.user?.isPrivate) await followStore.fetchPendingRequests();
  } catch (error) { console.error("Error fetching unread counts:", error); }
};

onMounted(fetchUnreadCounts);
</script>

<style scoped>
.fade-in { animation: fadeIn 0.15s ease-out; }
.slide-in-from-bottom-2 { animation: slideIn 0.15s ease-out; }
