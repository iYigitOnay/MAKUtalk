...
<template>
  ...
  >
    ...
    <button
      ...
      class="flex items-center gap-4 pt-4 pb-2 px-4 flex-shrink-0 outline-none group"
      :class="isExpanded ? 'justify-start' : 'justify-center'"
    >
      <div
        class="relative w-11 h-11 flex-shrink-0 group-hover:scale-105 transition-transform"
      >
        <img
      ...
      <span
        v-if="isExpanded"
        class="hidden lg:inline text-2xl font-black text-slate-900 dark:text-white tracking-tighter -ml-4"
      >
        MAKU<span
    ...
    <nav class="flex-1 space-y-1 py-2 px-2 sm:px-1">
      <button
        ...
        class="group flex items-center justify-center lg:justify-start gap-4 px-4 py-3 rounded-full text-slate-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-primary-900/20 hover:text-blue-600 dark:hover:text-primary-400 transition-all duration-200 w-full outline-none relative"
        :class="{
          'bg-blue-50 dark:bg-primary-900/20 text-blue-600 dark:text-primary-400':
            isActive(item.id),
          'opacity-60':
            !authStore.isAuthenticated && !['home', 'search'].includes(item.id),
        }"
      >
        ...
        <div
          v-if="isExpanded"
          class="hidden lg:flex items-center justify-between flex-1"
        >
          ...
          <svg
            v-if="
              ...
              !['home', 'search'].includes(item.id)
            "
            class="w-3.5 h-3.5 text-slate-400 opacity-50"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      >
        <div
          class="absolute inset-0 bg-gradient-to-r from-transparent via-white/40 to-transparent -translate-x-full group-hover:animate-shine pointer-events-none"
        ></div>
        <div
          class="w-6 h-6 flex-shrink-0 flex items-center justify-center text-blue-600 dark:text-primary-400"
        >
        ...
        >
          <span
            class="font-black italic text-lg tracking-normal bg-gradient-to-r from-blue-600 via-blue-500 to-purple-600 bg-clip-text text-transparent whitespace-nowrap pr-2 pb-1"
          >
            MAKÜ
          </span>
        </div>
    ...
    <div
      class="relative px-2 py-4 border-t border-slate-100 dark:border-gray-800"
    >
      <!-- GİRİŞ YAPMAMIŞ KULLANICI -->
      <button
        v-if="!authStore.isAuthenticated"
        @click="router.push('/auth')"
        class="w-full flex items-center gap-3 p-3 rounded-full bg-blue-600 hover:bg-blue-700 text-white transition-all duration-200 shadow-lg shadow-blue-500/20 active:scale-95"
      >
        <div
...
const navItems = [
  ...
  },
  {
    ...
    name: "Sohbet",
    iconSvg:
      '<svg fill="currentColor" viewBox="0 0 24 24"><path d="M20 2H4a2 2 0 00-2 2v18l4-4h14a2 2 0 002-2V4a2 2 0 00-2-2z" /></svg>',
    path: "/messages",
  },
];
...
const filteredNavItems = computed(() =>
  navItems.filter(
    (item) => item.id !== "requests" || authStore.user?.isPrivate,
  ),
);
...
const handleNavClick = async (item: any) => {
  // GİRİŞ KONTROLÜ
  if (!authStore.isAuthenticated && !["home", "search"].includes(item.id)) {
