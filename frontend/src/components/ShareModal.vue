<template>
  <Teleport to="body">
    <div v-if="isOpen" class="fixed inset-0 z-[999] flex items-end md:items-center justify-center p-0 md:p-4" @click.stop>
      <!-- Backdrop (Twitter Style Blur) -->
      <div class="absolute inset-0 bg-slate-950/40 backdrop-blur-md transition-opacity" @click="close"></div>
      ...
      >
        ...
        <div class="p-6 pb-0 flex items-center justify-between">
          <h3 class="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tighter italic">Paylaş</h3>
        ...
        <div class="p-6 grid grid-cols-2 gap-4">
          ...
          >
            <div class="w-10 h-10 bg-indigo-500/20 rounded-xl flex items-center justify-center text-indigo-500 group-hover:scale-110 transition-transform">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
            </div>
            <span class="text-xs font-black text-slate-900 dark:text-white uppercase tracking-widest">Kopyala</span>
          </button>
          ...
          >
            <div class="w-10 h-10 bg-emerald-500/20 rounded-xl flex items-center justify-center text-emerald-500 group-hover:scale-110 transition-transform">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" /></svg>
            </div>
            <span class="text-xs font-black text-slate-900 dark:text-white uppercase tracking-widest">Dışarıda</span>
          </button>
        </div>
            ...
            >
              <div class="flex items-center gap-3">
                <div class="relative">
                  ...
                  />
                  <div v-else class="w-10 h-10 rounded-full bg-indigo-500 flex items-center justify-center text-xs font-bold text-white">
                    {{ conv.otherParticipant?.username?.charAt(0).toUpperCase() }}
                  </div>
                </div>
                <div class="text-left min-w-0">
                  <p class="text-sm font-bold text-slate-900 dark:text-white truncate">@{{ conv.otherParticipant?.username }}</p>
                  <p class="text-[10px] text-slate-500 dark:text-slate-400 font-medium truncate">{{ conv.otherParticipant?.fullName || 'Öğrenci' }}</p>
                </div>
              </div>
              ...
              <button 
                ...
                :class="sentTo.includes(conv.id) ? 'bg-emerald-500 text-white' : 'bg-indigo-600 text-white hover:bg-indigo-500 hover:shadow-indigo-500/20'"
              >
                <span v-if="sentTo.includes(conv.id)" class="flex items-center gap-1">
                  <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" /></svg>
                  Gitti
                </span>
                <span v-else>{{ sendingTo === conv.id ? '...' : 'Gönder' }}</span>
              </button>
            </div>

            <!-- Boş Durum -->
            <div v-if="filteredConversations.length === 0 && !chatStore.loading" class="py-12 text-center opacity-40 italic text-sm">
              Arkadaş bulunamadı.
            </div>
          </div>
        </div>
...
const filteredConversations = computed(() => {
  if (!searchQuery.value.trim()) {
    return chatStore.conversations.filter(conv => conv.isAccepted).slice(0, 15);
  }
...
const sendToConversation = (conv: any) => {
  ...
  try {
    const text = shareComment.value.trim() || "Sana bir gönderi yolladı.";
    ...
    sendMessage(conv.id, encryptedContent, conv.otherParticipant.id, props.post.id);
