...
<template>
  ...
  >
    ...
    <div
      class="bg-white dark:bg-gray-900 w-full max-w-xl rounded-[2.5rem] shadow-2xl border border-gray-100 dark:border-primary-900/20 relative z-10 animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col max-h-[90vh]"
    >
      <!-- Header -->
      <div
        class="p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between"
      >
        <h3
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div
          v-if="showCropper"
          class="bg-slate-50 dark:bg-gray-800 rounded-3xl p-4 space-y-4 animate-in slide-in-from-top-4 duration-300"
        >
          ...
          <div
            class="h-64 sm:h-80 w-full rounded-2xl overflow-hidden border-2 border-dashed border-gray-200 dark:border-gray-700"
          >
            <cropper
              ref="cropperRef"
              class="h-full w-full"
        ...
        <div v-else class="relative mb-16">
          ...
          >
            <img
              v-if="coverPreview || editData.coverUrl"
              :src="coverPreview || getImageUrl(editData.coverUrl)"
            ...
            >
              <span
                class="text-white text-[10px] font-black bg-black/40 px-4 py-2 rounded-full backdrop-blur-md uppercase tracking-widest border border-white/20"
                >Kapak Fotoğrafını Değiştir</span
              >
            </div>
          </div>
          ...
          <div class="absolute -bottom-10 left-6">
            ...
            <div
              ...
              class="rounded-full shadow-xl relative group cursor-pointer"
            >
              <UserAvatar
                :user="{
                  ...editData,
                  avatarUrl: avatarPreview || editData.avatarUrl,
                }"
                size="2xl"
                customClass="border-4 border-white dark:border-gray-900"
              />
              <div
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
          <div class="space-y-1.5">
            ...
            <input
              ...
              type="text"
              class="w-full px-4 py-3 rounded-2xl bg-gray-50 dark:bg-gray-800 border-2 border-transparent focus:border-blue-500 outline-none transition-all text-sm font-black"
          ...
          <div class="space-y-1.5">
            ...
            <input
              ...
              type="text"
              disabled
              class="w-full px-4 py-3 rounded-2xl bg-gray-100 dark:bg-gray-800/50 border-2 border-transparent text-gray-400 outline-none text-sm font-black cursor-not-allowed"
            />
          </div>
          <div class="space-y-1.5">
            <label
        ...
        <div class="space-y-1.5">
          ...
          <textarea
            ...
            rows="3"
            class="w-full px-4 py-3 rounded-2xl bg-gray-50 dark:bg-gray-800 border-2 border-transparent focus:border-blue-500 outline-none transition-all text-sm font-black resize-none"
        ...
        <!-- Privacy Toggle -->
        <div
          class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-2xl border-2 border-transparent hover:border-blue-100 dark:hover:border-blue-900/30 transition-all"
        >
          ...
          >
            <div
              ...
              :class="editData.isPrivate ? 'translate-x-6' : 'translate-x-0'"
            ></div>
          </button>
        </div>
      </div>
      ...
      <!-- Footer -->
      <div
        class="p-6 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30"
      >
        <button
          @click="handleSave"
...
watch(
  ...
  (newVal) => {
    if (newVal && props.user) {
      ...
      userInitial.value = (props.user.fullName || props.user.username || "?")
        .charAt(0)
        .toUpperCase();
    }
  },
...
const handleSave = async () => {
  loading.value = true;
  try {
    console.log('[EditProfileModal] Kaydedilen veriler:', {
      fullName: editData.value.fullName,
      isPrivate: editData.value.isPrivate,
      isPrivateType: typeof editData.value.isPrivate
    });
