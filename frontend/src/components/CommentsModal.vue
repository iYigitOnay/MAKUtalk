...
<template>
  <Teleport to="body">
    <transition name="modal">
      <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/40 backdrop-blur-md" @click.self="closeMainModal">
        <div class="bg-white dark:bg-gray-950 w-full max-w-2xl sm:rounded-[2.5rem] shadow-2xl flex flex-col border-t sm:border border-gray-100 dark:border-white/5 max-h-[90vh] sm:max-h-[80vh] overflow-hidden animate-slide-up relative">
          
          <!-- MODERATION OVERLAYS -->
          <!-- 1. CUSTOM DELETE CONFIRMATION -->
          <transition name="fade">
            <div v-if="showDeleteConfirm" class="absolute inset-0 z-[110] bg-white/90 dark:bg-gray-950/90 backdrop-blur-xl flex items-center justify-center p-8">
              <div class="text-center max-w-xs animate-in zoom-in-95 duration-200">
                ...
                <h3 class="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tighter">Yorumu Sil?</h3>
                <p class="text-gray-500 text-sm mt-2 mb-8">Bu işlem geri alınamaz. Yorum tamamen kaldırılacaktır.</p>
                <div class="flex flex-col gap-3">
                  ...
                  <button @click="showDeleteConfirm = false" class="text-xs font-black text-gray-400 uppercase tracking-widest hover:text-gray-600 transition-colors">VAZGEÇ</button>
                </div>
              </div>
            </div>
          </transition>
          ...
          <div class="flex-1 overflow-y-auto p-4 sm:p-8 space-y-6 no-scrollbar">
            <div v-if="commentsStore.loading && !commentsStore.comments.length" class="py-20 text-center"><div class="w-10 h-10 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mx-auto"></div></div>
            <div v-else-if="!commentsStore.comments.length" class="py-20 text-center opacity-40 italic font-bold text-gray-500 uppercase text-xs tracking-widest">Henüz yorum yapılmamış. İlk yorumu sen yap! ✨</div>
                ...
                <div class="bg-slate-50 dark:bg-gray-900/50 p-4 rounded-2xl rounded-tl-none border border-gray-100 dark:border-white/5 relative">
                  <div class="flex items-center justify-between mb-1 gap-2">
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <!-- ACTIONS -->
                      <button v-if="authStore.user?.id === comment.userId || isAdmin" @click="openDelete(comment.id)" class="text-gray-400 hover:text-red-500 transition-colors" :title="isAdmin ? 'Yönetici Olarak Sil' : 'Yorumu Sil'">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                      </button>

                      <button v-if="authStore.user?.id !== comment.userId" @click="openReport(comment.id)" class="text-gray-400 hover:text-orange-500 transition-colors" title="Rapor Et">
          ...
          <div v-if="authStore.isAuthenticated" class="p-4 sm:p-8 bg-slate-50 dark:bg-gray-900/50 border-t border-gray-100 dark:border-white/5">
            <form @submit.prevent="handleSubmit" class="flex items-center gap-3">
              <input v-model="newComment" type="text" placeholder="Sen ne düşünüyorsun?" class="flex-1 h-12 px-5 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm rounded-2xl border border-gray-200 dark:border-white/5 outline-none focus:ring-2 focus:ring-blue-500/20 transition-all" :disabled="commentsStore.loading" />
              <button type="submit" :disabled="!newComment.trim() || commentsStore.loading" class="h-12 px-6 bg-blue-600 text-white text-xs font-black rounded-2xl uppercase hover:bg-blue-700 disabled:opacity-50 transition-all active:scale-95 shadow-lg shadow-blue-500/20 flex items-center justify-center gap-2">
...
interface Props { isOpen: boolean; postId: number | null; }
...
watch(() => props.isOpen, (isOpen) => { if (isOpen && props.postId) commentsStore.fetchComments(props.postId); });

const handleSubmit = async () => {
  if (!props.postId || !newComment.value.trim()) return;
  try { await commentsStore.createComment(props.postId, newComment.value); newComment.value = ""; toast.success("Yorumun eklendi! 💬"); }
  catch (error: any) { toast.error(error.message || "Yorum eklenemedi."); }
};

// DELETE ACTIONS
const openDelete = (id: number) => { commentToDelete.value = id; showDeleteConfirm.value = true; };
const confirmDelete = async () => {
  if (!commentToDelete.value) return;
  try { await commentsStore.deleteComment(commentToDelete.value); toast.success("Yorum silindi."); }
  catch { toast.error("Silme işlemi başarısız."); }
  finally { showDeleteConfirm.value = false; commentToDelete.value = null; }
};

// REPORT ACTIONS
const openReport = (id: number) => { commentToReport.value = id; showReportModal.value = true; reportStep.value = 1; };
...
const submitReport = async (subReason: string) => {
  try {
    await apiClient.post('/users/report', { reportedCommentId: commentToReport.value, reason: selectedCategory.value, subReason });
    toast.warning("Yorum şikayet edildi. İncelenecektir.");
  } catch { toast.error("Şikayet iletilemedi."); }
  finally { closeReport(); }
};
const closeReport = () => { showReportModal.value = false; commentToReport.value = null; reportStep.value = 1; };

const closeMainModal = () => { if (!showDeleteConfirm.value && !showReportModal.value) emit('close'); };
