...
<template>
  <div
    class="min-h-screen bg-white dark:bg-gray-950 transition-colors duration-300 pb-20"
  >
    <!-- Standard Minimal Header -->
    <header
      class="sticky top-0 z-50 bg-white/80 dark:bg-gray-950/80 backdrop-blur-md border-b border-gray-100 dark:border-white/[0.05] px-4 py-3 flex items-center gap-6"
    >
      <button
        @click="$router.back()"
        class="p-2 hover:bg-gray-100 dark:hover:bg-white/5 rounded-full transition-all text-gray-900 dark:text-white active:scale-90"
      >
        <svg
          class="w-5 h-5"
          fill="none"
      ...
      <h1
        class="text-base font-black text-gray-900 dark:text-white uppercase tracking-tight italic"
      >
        Gönderi
      </h1>
    </header>
    ...
    <div
      v-else-if="post"
      class="max-w-2xl mx-auto animate-in fade-in duration-300"
    >
      <!-- 1. ANCESTOR THREAD -->
      ...
      >
        ...
        <div class="flex items-center justify-between mb-3">
            ...
            <div class="flex flex-col">
              ...
              >
                <router-link
                  ...
                  class="font-black text-[15px] hover:underline"
                >
                ...
                >
                  ...
                  <div
                    v-for="ub in post.author.badges"
              ...
              <span class="text-sm text-gray-500 font-medium leading-none"
          ...
          <div class="flex items-center gap-2">
            <span
              v-if="post.isAcademic"
              class="px-2 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 text-[9px] font-black rounded-lg uppercase tracking-wider border border-emerald-200 dark:border-emerald-800"
              >AKADEMİK DUYURU</span
            >
            <div class="relative">
              <button
        ...
        <div class="flex flex-wrap gap-1.5 mb-2.5 px-1">
          <span
            v-if="post.category"
            class="px-1.5 py-0.5 rounded-full text-[8px] font-black uppercase text-white"
            :style="{ backgroundColor: post.category.color || '#3b82f6' }"
            >{{ post.category.name }}</span
          >
          <span
            v-if="post.sentiment"
            class="px-1.5 py-0.5 rounded-full text-[8px] font-black uppercase border"
            :class="getSentimentStyles(post.sentiment).class"
            >{{ translateSentiment(post.sentiment) }}</span
          >
        </div>

        <div class="mb-4">
          <p
            class="text-[15px] sm:text-[16px] text-gray-900 dark:text-gray-100 leading-relaxed whitespace-pre-wrap px-1"
          >
        ...
        <div v-if="post.documentUrl" class="mb-4">
          <a
            :href="getImageUrl(post.documentUrl)"
            target="_blank"
            download
          ...
          >
            ...
            <div
              ...
              :class="{
                'bg-red-500': getFileExt(post.documentUrl) === 'pdf',
                'bg-blue-500': ['doc', 'docx'].includes(
                  getFileExt(post.documentUrl),
                ),
                ...
                'bg-orange-500': ![
                  ...
                  'xls',
                  'xlsx',
                ].includes(getFileExt(post.documentUrl)),
              }"
            ></div>
            ...
            <div
              ...
              :class="{
                'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400':
                  getFileExt(post.documentUrl) === 'pdf',
                'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400':
                  ['doc', 'docx'].includes(getFileExt(post.documentUrl)),
                'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400':
                  ['xls', 'xlsx'].includes(getFileExt(post.documentUrl)),
        ...
        <div
          class="flex items-center gap-2 text-[11px] font-bold text-gray-400 border-b border-gray-50 dark:border-white/[0.03] pb-4 mb-4 px-1"
        >
          <span>{{ formatDate(post.createdAt, true) }}</span>
          <span class="opacity-30">·</span>
          <span
            :class="post.isAcademic ? 'text-emerald-600' : 'text-blue-600'"
            class="font-black"
            >{{ post.isAcademic ? "MAKU Akademik" : "MAKU Talk" }}</span
          >
        </div>
        ...
        <div class="flex items-center justify-between px-2">
          ...
          >
            ...
            >
              <component
                v-if="post.isAcademic"
                :is="getBadgeComponent('check-circle')"
                class="w-5 h-5 transition-transform group-active:scale-125"
                :class="post.isLiked ? 'text-emerald-600' : ''"
              />
              <component
                v-else
          ...
          <button
            ...
            class="flex items-center gap-2 group text-gray-500 hover:text-blue-600 transition-all"
          >
            <div
              class="p-2 rounded-full group-hover:bg-blue-50 dark:group-hover:bg-blue-900/20 transition-all"
            >
              <component
          ...
          >
            <div
              class="p-2 rounded-full group-hover:bg-yellow-50 dark:group-hover:bg-yellow-900/20 transition-all"
            >
              <component
                :is="getBadgeComponent('bookmark')"
                class="w-5 h-5 transition-transform group-active:scale-125"
                :class="post.isBookmarked ? 'fill-yellow-600' : 'fill-none'"
              />
            </div>
          </button>
          <button
            ...
            :class="post.isReposted ? 'text-green-600' : 'text-gray-500'"
          >
            <div
              class="p-2 rounded-full group-hover:bg-green-50 dark:group-hover:bg-green-900/20 transition-all"
            >
              <component
          ...
          >
            <component
              ...
              class="w-5 h-5 transition-transform group-active:scale-125 text-gray-500 dark:text-gray-400 group-hover:text-current"
            />
          </button>
        </div>
      </article>
        ...
        <div class="flex gap-3">
          ...
          <div class="flex-1">
            ...
            <div
              ...
              class="relative mt-2 mb-4 inline-block"
            >
              <img
                :src="selectedImagePreview"
                class="max-h-48 rounded-2xl border border-gray-100 dark:border-white/10 shadow-lg shadow-blue-500/5"
              />
              <button
                @click="removeSelectedImage"
                class="absolute -top-2 -right-2 p-1 bg-gray-900/80 text-white rounded-full hover:bg-black transition-all"
              >
            ...
            <div
              class="flex items-center justify-between pt-2 border-t border-slate-50 dark:border-white/[0.03] relative"
            >
              <div class="flex items-center gap-1">
                <button
                  @click="triggerFileInput"
                  class="p-2 text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-full transition-all group"
                  title="Resim Ekle"
                >
              ...
              <div class="flex items-center gap-4">
                    ...
                    >
                      <svg class="w-full h-full -rotate-90">
                        ...
                        <circle
                          ...
                          :stroke-dashoffset="
                            63 -
                            (Math.min(commentContent.length, 280) / 280) * 63
                          "
                          stroke-linecap="round"
                        />
                      </svg>
                      <span
                        v-if="commentContent.length > 240"
                        ...
                        :class="
                          commentContent.length >= 280
                            ...
                            : 'text-gray-400'
                        "
                        >{{ 280 - commentContent.length }}</span
                      >
                    </div>
                    <div
                      v-if="commentContent.length >= 280"
                ...
                <button
                  @click="submitReply"
                  :disabled="
                    (!commentContent.trim() && !selectedImage) ||
                    commentContent.length > 280 ||
                    commentLoading
                  "
                  class="px-5 py-1.5 bg-blue-600 text-white font-black text-xs rounded-full hover:bg-blue-700 transition-all disabled:opacity-40 shadow-lg shadow-blue-500/10 active:scale-95"
                >
      ...
      <div
        v-else
        class="py-20 text-center opacity-30 italic font-black uppercase text-[10px] tracking-[0.3em]"
      >
        Henüz yanıt yok
      </div>
    </div>
      ...
      <transition name="fade">
        ...
        >
          ...
          >
            ...
            <div
              class="flex-1 overflow-y-auto p-4 space-y-1.5 max-h-[60vh] no-scrollbar text-left"
            >
              <div v-if="reportStep === 1">
                <button
                  v-for="(subs, cat) in reportCategories"
                  :key="cat"
                  @click="selectReportCategory(cat)"
                  class="w-full text-left p-4 rounded-2xl hover:bg-slate-50 dark:hover:bg-white/5 transition-all font-bold text-gray-700 dark:text-gray-300 flex items-center justify-between group"
                >
                  {{ cat }}
                  <component
              ...
              <div v-else>
                ...
                <button
                  ...
                  class="w-full text-left p-4 rounded-2xl hover:bg-red-50 dark:hover:bg-red-900/20 group font-bold text-gray-700 dark:text-gray-300"
                >
                  {{ sub }}
                </button>
              </div>
...
const submitReply = async () => {
  if (
    (!commentContent.value.trim() && !selectedImage.value) ||
    commentContent.value.length > 280 ||
    !post.value
  )
    return;
...
const handleBookmark = async () => {
  if (!post.value || !authStore.isAuthenticated) return;
  try {
    const res = await postsStore.toggleBookmark(post.value.id);
    if (res.bookmarked) toast.success("Kaydedildi! 🔖");
  } catch {
    toast.error("Hata!");
  }
};
