...
<template>
  ...
  >
          ...
          <div class="relative z-10">
            <div class="mb-4 md:mb-6 flex justify-center">
              <img
                ...
                :class="activeTab === 'register' ? '-scale-x-100 rotate-y-180 opacity-80' : 'scale-x-100'"
              />
            </div>
            ...
            <p class="text-slate-500 dark:text-white/60 text-sm md:text-sm leading-relaxed max-w-[250px] font-medium mx-auto">
              {{ activeTab === "login" ? "Dijital Kampüs dünyasına hoş geldin!" : "Topluluğun bir parçası olmaya hazır mısın?" }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="relative flex-1 md:w-full min-h-[450px] md:min-h-[600px] flex overflow-hidden">
        ...
        <div v-if="showVerification" class="absolute inset-0 z-30 bg-white/98 dark:bg-[#161b27]/98 backdrop-blur-md flex items-center justify-center p-6 transition-all duration-500">
          <div class="w-full max-w-[340px] text-center pb-20 md:pb-0">
            <div class="mb-6 inline-flex p-4 rounded-full bg-blue-50 dark:bg-blue-900/20 text-blue-600">
            ...
            <form @submit.prevent="handleVerify" class="space-y-6">
              ...
              <button type="button" @click="showVerification = false" class="text-slate-400 text-sm font-bold block w-full mt-4 hover:text-blue-500 transition-colors">Geri Dön</button>
            </form>
          </div>
        </div>

        <!-- Forgot Password Step -->
        <div v-if="showForgotModal" class="absolute inset-0 z-40 bg-white/98 dark:bg-[#161b27]/98 backdrop-blur-md flex items-center justify-center p-6 transition-all duration-500">
          <div class="w-full max-w-[340px] text-center pb-20 md:pb-0">
            <div class="mb-6 inline-flex p-4 rounded-full bg-red-50 dark:bg-red-900/20 text-red-600">
            ...
            <form v-else @submit.prevent="handleResetPassword" class="space-y-4 text-left">
              ...
              <input v-model="forgotForm.confirmPassword" type="password" placeholder="Tekrar" class="auth-input shadow-inner" />
              <button type="submit" :disabled="forgotLoading || !isResetValid" class="auth-btn">Güncelle</button>
              <button type="button" @click="forgotStep = 1" class="text-slate-400 text-sm font-bold block w-full text-center mt-4">E-posta Değiştir</button>
            </form>
          </div>
        </div>
            ...
            <Transition mode="out-in" enter-active-class="transition duration-500 ease-out" enter-from-class="opacity-0 translate-x-12" enter-to-class="opacity-100 translate-x-0" leave-active-class="transition duration-400 ease-in" leave-from-class="opacity-100 translate-x-0" leave-to-class="opacity-0 -translate-x-12">
              ...
              <div v-if="!isReviewing" key="inputs" class="space-y-6">
                ...
                <form @submit.prevent="isReviewing = true" class="space-y-4">
                  ...
                  <div class="relative group">
                    <input v-model="registerForm.password" :type="showPassword ? 'text' : 'password'" placeholder="Şifre" class="auth-input shadow-inner" />
                  ...
                  <div class="grid grid-cols-2 gap-x-2 gap-y-1.5 px-1 py-1">
                    ...
                    <div class="flex items-center gap-1.5 justify-end text-right">
                      <span class="text-[8px] font-black uppercase tracking-tighter" :class="passChecks['Kullanıcı Adı İçeremez'] ? 'text-emerald-500' : (registerForm.password ? 'text-red-500' : 'text-gray-400')">Kullanıcı Adı İçeremez</span>
                      <div class="w-1.5 h-1.5 rounded-full transition-colors" :class="passChecks['Kullanıcı Adı İçeremez'] ? 'bg-emerald-500' : (registerForm.password ? 'bg-red-500' : 'bg-gray-300 dark:bg-gray-700')"></div>
                    </div>
                  </div>

                  <button type="submit" :disabled="registerLoading || hasErrors" class="auth-btn mt-2 shadow-blue-500/10">Hesap Oluştur</button>
                  <p class="text-center text-gray-500 dark:text-white/30 text-xs mt-4">Zaten üye misin? <button type="button" @click="activeTab = 'login'" class="text-blue-600 font-black hover:underline transition-all">Giriş Yap</button></p>
                </form>
              </div>
              ...
              <div v-else key="review" class="space-y-10 py-4 text-left px-2 relative">
                ...
                <div class="relative space-y-12 ml-4">
                  <!-- Dynamic Line -->
                  <div class="absolute left-[11px] top-3 bottom-3 w-0.5 bg-slate-100 dark:bg-white/5 overflow-hidden">
                    <div class="absolute top-0 left-0 w-full bg-gradient-to-b from-emerald-500 via-blue-500 to-purple-500 transition-all duration-700" :style="{ height: (28 + (agreementsAccepted ? 36 : 0) + (marketingConsent ? 36 : 0)) + '%' }"></div>
                  </div>
                  ...
                  <div class="relative flex items-start gap-6 cursor-pointer group" @click="agreementsAccepted = !agreementsAccepted">
                    <div class="z-10 mt-1 w-6 h-6 rounded-full transition-all duration-500 flex items-center justify-center border-4 border-white dark:border-[#161b27] shadow-lg" :class="agreementsAccepted ? 'bg-blue-600 shadow-blue-500/20' : 'bg-slate-200 dark:bg-white/10'"><svg class="w-3 h-3 text-white" v-if="agreementsAccepted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="4" d="M5 13l4 4L19 7"/></svg></div>
                    <div class="flex-1">
                  ...
                  <div class="relative flex items-start gap-6 cursor-pointer group" @click="marketingConsent = !marketingConsent">
                    <div class="z-10 mt-1 w-6 h-6 rounded-full transition-all duration-500 flex items-center justify-center border-4 border-white dark:border-[#161b27] shadow-lg" :class="marketingConsent ? 'bg-purple-600 shadow-purple-500/20' : 'bg-slate-200 dark:bg-white/10'"><svg class="w-3 h-3 text-white" v-if="marketingConsent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="4" d="M5 13l4 4L19 7"/></svg></div>
                    <div class="flex-1">
                      <h4 class="text-sm font-black transition-colors" :class="marketingConsent ? 'text-purple-600 dark:text-purple-400' : 'text-slate-400'">İletişim Tercihi</h4>
                      <p class="text-[11px] font-bold mt-1" :class="marketingConsent ? 'text-slate-700 dark:text-white/80' : 'text-slate-400/60'">Kampüs yenilikleri için <button type="button" @click.stop="openLegal('etk')" class="bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent hover:underline">E-İleti</button> almayı kabul ediyorum.</p>
                    </div>
                  </div>
                </div>

                <div class="flex flex-col gap-4 pt-8 text-center">
                  <button @click="handleRegister" :disabled="registerLoading || !agreementsAccepted" class="auth-btn py-4 shadow-xl overflow-hidden group">
                    <span class="relative z-10">{{ registerLoading ? 'Sisteme Sızılıyor...' : 'Yolculuğu Başlat' }}</span>
                    <div class="absolute inset-0 bg-white/10 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
                  </button>
                  <button @click="isReviewing = false" class="text-[10px] font-black text-slate-400 hover:text-blue-600 transition-all uppercase tracking-[0.3em] italic">← Bilgileri Düzenle</button>
                </div>
              </div>
            </Transition>
          </div>
        ...
        <div
          ...
          :class="[ activeTab === 'register' ? 'md:opacity-0 md:invisible hidden md:flex' : 'opacity-100 visible flex' ]"
        >
          <div class="w-full max-w-[340px]">
            <h2 class="text-2xl font-black text-gray-900 dark:text-white mb-6 text-center italic tracking-tighter uppercase leading-none">Tekrar Hoş Geldin</h2>
            <form @submit.prevent="handleLogin" class="space-y-4">
              <div class="relative flex items-center">
                <input v-model="loginForm.email" type="text" placeholder="E-posta veya Öğrenci No" class="auth-input shadow-inner" />
                <span v-if="loginForm.email && !loginForm.email.includes('@')" class="absolute right-4 text-[9px] font-black text-slate-400 uppercase tracking-tighter pointer-events-none select-none">@ogr.mehmetakif.edu.tr</span>
              </div>
              <div class="relative group">
              ...
              <button type="submit" :disabled="loginLoading" class="auth-btn mt-2">Giriş Yap</button>
              <div class="flex justify-between mt-4 px-1">
                <button type="button" @click="showForgotModal = true; forgotStep = 1" class="text-xs font-black text-slate-400 hover:bg-gradient-to-r hover:from-blue-400 hover:to-purple-400 hover:bg-clip-text hover:text-transparent transition-all duration-300">Şifremi Unuttum</button>
                <button type="button" @click="activeTab = 'register'" class="text-blue-600 text-xs font-black hover:underline transition-all">Kayıt Ol</button>
              </div>
            </form>
          </div>
        </div>
    ...
    <!-- Legal Modals -->
    <LegalModal :is-open="legalModal.isOpen" :title="legalModal.title" :type="legalModal.type" @close="legalModal.isOpen = false" />
  </div>
</template>

<style scoped>
.auth-input { @apply w-full px-4 py-3 rounded-xl bg-gray-100 dark:bg-[#1a1f2e] border border-gray-200 dark:border-white/[0.07] text-gray-900 dark:text-white outline-none focus:border-blue-500/50 transition-all text-sm placeholder-gray-400; }
.auth-btn { @apply relative w-full py-3.5 rounded-xl text-white text-sm font-black bg-gradient-to-r from-blue-600 to-purple-600 shadow-lg active:scale-95 transition-all disabled:opacity-50; }
</style>
...
const openLegal = (type: any) => {
  const titles = { terms: "Kullanım Koşulları", privacy: "Gizlilik Politikası", kvkk: "KVKK Aydınlatma Metni", etk: "Elektronik İleti İzni" };
...
const passChecks = computed(() => ({
  ...
  "Kullanıcı Adı İçeremez": registerForm.value.username ? !registerForm.value.password.toLowerCase().includes(registerForm.value.username.toLowerCase()) : true,
}));

const isResetValid = computed(() => forgotForm.value.newPassword.length >= 8 && forgotForm.value.newPassword === forgotForm.value.confirmPassword);
...
const checkUsername = async () => {
  ...
  usernameCheckTimeout = setTimeout(async () => {
    ...
    } catch { isUsernameTaken.value = false; }
  }, 500);
};

const usernameError = computed(() => isUsernameTaken.value ? "Bu kullanıcı adı alınmış!" : null);
const hasErrors = computed(() => !registerForm.value.fullName || !!usernameError.value || !Object.values(passChecks.value).every(v => v));

const handleLogin = async () => {
  if (!loginForm.value.email || !loginForm.value.password) { toast.info("Lütfen tüm alanları doldurun."); return; }
  loginLoading.value = true;
  try {
    const email = loginForm.value.email.includes('@') ? loginForm.value.email : `${loginForm.value.email}@ogr.mehmetakif.edu.tr`;
...
const handleSendResetCode = async () => {
  if (!forgotForm.value.email) { toast.info("E-posta adresi girin."); return; }
