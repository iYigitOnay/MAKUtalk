...
<template>
  <div class="flex h-screen bg-[#fdfdff] dark:bg-[#0b0f19] overflow-hidden font-sans relative text-left">
    ...
    <aside 
      class="w-full md:w-[250px] lg:w-[300px] flex-shrink-0 border-r border-slate-200/60 dark:border-white/5 flex flex-col bg-white dark:bg-[#0f172a]/40 backdrop-blur-xl transition-all duration-300 h-full"
    ...
    >
      <div class="p-6 pb-4">
        ...
        <div class="relative group">
          <input type="text" placeholder="Kişilerde ara..." class="w-full bg-slate-100/50 dark:bg-white/5 border-none rounded-2xl py-3 pl-11 pr-4 text-sm focus:ring-2 focus:ring-indigo-500/50 transition-all outline-none text-slate-900 dark:text-white" />
          <svg class="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-indigo-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto px-3 pb-6 space-y-1 custom-scrollbar">
        <div v-if="chatStore.loading && !chatStore.conversations.length" class="space-y-2">
          <div v-for="i in 5" :key="i" class="flex items-center gap-4 p-4 animate-pulse bg-slate-50 dark:bg-white/5 rounded-2xl">
            <div class="w-12 h-12 bg-slate-200 dark:bg-white/10 rounded-xl"></div>
        ...
        <button
          ...
          class="w-full p-4 flex items-center gap-4 rounded-2xl transition-all duration-300 relative border border-transparent group text-left"
        ...
        >
          <div class="relative flex-shrink-0 w-12 h-12">
            ...
            <div v-else class="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold">{{ conv.otherParticipant?.username?.charAt(0).toUpperCase() }}</div>
          </div>
    ...
    <main class="flex-1 flex flex-col bg-white dark:bg-[#0b0f19] relative h-full">
      <div v-if="chatStore.activeConversation" class="flex flex-col h-full overflow-hidden">
        ...
        <header class="h-20 px-8 flex items-center justify-between border-b border-slate-200/60 dark:border-white/5 bg-white/80 dark:bg-[#0b0f19]/80 backdrop-blur-xl z-20 flex-shrink-0">
          <div class="flex items-center gap-4">
            ...
            <div class="flex items-center gap-4">
              <div class="relative">
                ...
                <div class="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-emerald-500 border-2 border-white dark:border-[#0b0f19] rounded-full shadow-sm"></div>
              </div>
              <div>
          ...
          <div class="flex items-center gap-2">
            <div class="relative">
              ...
              <div v-if="showMoreMenu" class="absolute right-0 mt-2 w-64 bg-white dark:bg-gray-900 border border-slate-100 dark:border-white/10 rounded-[2rem] shadow-2xl z-50 py-4 animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
                ...
                <div class="px-6 pb-2">
                  <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Sohbet Teması</p>
                  <div class="grid grid-cols-6 gap-2">
                    <button v-for="t in themes" :key="t.color" @click="setTheme(t.color)" class="w-6 h-6 rounded-full border-2 border-white dark:border-gray-800 transition-transform hover:scale-125 shadow-sm" :style="{ backgroundColor: t.color }"></button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div ref="messagesContainer" class="flex-1 overflow-y-auto px-6 py-8 space-y-4 custom-scrollbar bg-slate-50/20 dark:bg-transparent">
          <!-- İSTEK PANELİ -->
          <div v-if="!chatStore.activeConversation.isAccepted && !chatStore.activeConversation.isRejected" class="flex flex-col items-center justify-center py-10 px-6 bg-amber-50/50 dark:bg-amber-900/10 rounded-[3rem] border border-amber-100 dark:border-amber-900/30 mb-8 animate-in zoom-in-95 duration-500">
            <div class="w-16 h-16 bg-amber-500 text-white rounded-[1.5rem] flex items-center justify-center text-3xl mb-4 shadow-lg">✉️</div>
            <h4 class="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tighter mb-2 text-center">Mesaj İsteği</h4>
            <template v-if="isMyMessage(chatStore.messages[0]?.senderId)">
              <p class="text-xs text-slate-500 text-center max-w-xs font-medium leading-relaxed italic">Karşı tarafın onaylaması bekleniyor.</p>
            </template>
            <template v-else>
              <p class="text-xs text-slate-500 text-center max-w-xs font-medium mb-6 leading-relaxed">Bu kullanıcı ilanınız için mesaj gönderdi. Sohbeti başlatın?</p>
              <div class="flex gap-3 w-full max-w-xs">
                ...
                <button @click="handleAcceptRequest" class="flex-1 py-3.5 bg-emerald-500 text-white rounded-2xl text-[10px] font-black uppercase shadow-lg active:scale-95 transition-all">Kabul Et</button>
              </div>
            </template>
          </div>

          <div v-if="chatStore.activeConversation.isRejected" class="flex flex-col items-center justify-center py-10 px-6 opacity-60">
            <div class="w-16 h-16 bg-red-100 text-red-500 rounded-full flex items-center justify-center text-3xl mb-4">🚫</div>
            <p class="text-xs font-black text-red-500 uppercase tracking-widest text-center">{{ isMyMessage(chatStore.messages[0]?.senderId) ? 'İlan sahibi isteği reddetti.' : 'Bu isteği reddettiniz.' }}</p>
          </div>
          ...
          <div v-for="(msg, index) in chatStore.messages" :key="msg.id || index" class="flex flex-col" :class="isMyMessage(msg.senderId) ? 'items-end' : 'items-start'">
            <div class="max-w-[85%] md:max-w-[70%] px-5 py-3.5 text-sm font-medium rounded-3xl shadow-sm" :class="isMyMessage(msg.senderId) ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white dark:bg-[#1e293b] text-slate-900 dark:text-slate-100 rounded-tl-none border border-slate-100 dark:border-white/5'" :style="isMyMessage(msg.senderId) ? { backgroundColor: currentThemeColor } : {}">
              {{ decrypt(msg.content) }}
            </div>
          </div>
        </div>
        ...
        <footer class="p-4 md:p-6 bg-white dark:bg-[#0b0f19] border-t border-slate-200/60 dark:border-white/5 flex-shrink-0">
          <div v-if="chatStore.activeConversation.isAccepted" class="max-w-4xl mx-auto flex items-end gap-3">
...
const reportCategories: Record<string, string[]> = {
  ...
  "Taciz ve Rahatsızlık": ["Hakaret", "İstenmeyen Cinsel İçerik", "Açıkça Nesneleştirme", "İstenmeyen Yetişkinlere Yönelik İçerik", "Hedefli Taciz"],
...
const setTheme = async (color: string) => {
  ...
  try { await apiClient.post(`/chat/theme/${chatStore.activeConversation.id}`, { color }); currentThemeColor.value = color; chatStore.activeConversation.themeColor = color; showMoreMenu.value = false; toast.success("Sohbet rengi güncellendi ✨"); } catch { toast.error("Hata!"); }
};
...
const handleAcceptRequest = async () => {
  if (!chatStore.activeConversation) return;
  try { await apiClient.post(`/chat/accept/${chatStore.activeConversation.id}`); chatStore.activeConversation.isAccepted = true; toast.success("İstek kabul edildi! ✨"); } catch { toast.error("İşlem başarısız."); }
};
...
onMounted(async () => {
  ...
  if (route.query.userId) {
    await chatStore.selectConversation(Number(route.query.userId), route.query.fromSpot === 'true');
