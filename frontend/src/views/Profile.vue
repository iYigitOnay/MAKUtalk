...
<template>
  <div class="max-w-4xl mx-auto pb-20 text-left">
    ...
    <template v-else-if="displayedUser">
      ...
      <div class="relative">
        ...
        <div class="absolute -bottom-10 sm:-bottom-16 left-4 sm:left-6">
          <div @click="displayedUser.avatarUrl ? (showAvatarZoom = true) : null" class="relative group p-1 rounded-full bg-gradient-to-br from-blue-400 to-purple-400 shadow-xl overflow-hidden transition-transform duration-300 hover:scale-105 active:scale-95" :class="[displayedUser.avatarUrl ? 'cursor-pointer' : '']">
            ...
            <!-- BANNED OVERLAY ICON -->
            <div v-if="displayedUser.isBanned" class="absolute inset-0 flex items-center justify-center bg-black/40 rounded-full">
              <component :is="getBadgeComponent('shield-off')" class="w-8 h-8 sm:w-12 sm:h-12 text-white drop-shadow-lg" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-gray-900/40 rounded-b-xl border-x border-b border-slate-200 dark:border-primary-900/30 pt-12 sm:pt-20 px-4 sm:px-6 pb-6 shadow-sm">
        <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
          <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-2 mb-1">
              <h1 class="text-lg sm:text-2xl font-black text-slate-900 dark:text-white truncate max-w-[200px] sm:max-w-full" :class="{'text-gray-400 line-through decoration-red-500': displayedUser.isBanned}">{{ displayedUser.fullName || displayedUser.username }}</h1>
              
              <!-- BADGES -->
              <div v-if="(displayedUser.badges?.length || displayedUser.role === 'ADMIN') && !displayedUser.isBanned" class="flex flex-wrap gap-1 items-center">
                <div v-if="displayedUser.role === 'ADMIN'" class="p-1 rounded-full text-white bg-[#1E3A8A] shadow-sm group relative">
                  <component :is="getBadgeComponent('crown')" class="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5" />
                  <div class="absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-900 text-white px-2 py-1 rounded text-[8px] font-black opacity-0 group-hover:opacity-100 transition-all whitespace-nowrap z-[100] pointer-events-none uppercase">Sistem Kurucusu</div>
                </div>
                <div v-for="ub in displayedUser.badges" :key="ub.badge?.id || ub.id" class="p-1 rounded-full border border-gray-100 group relative" :style="{ backgroundColor: ub.badge?.color || '#3b82f6', color: 'white' }">
                  <component :is="getBadgeComponent(ub.badge?.icon || 'award')" class="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5" />
                  <div class="absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-900 text-white px-2 py-1 rounded text-[8px] font-black opacity-0 group-hover:opacity-100 transition-all whitespace-nowrap z-[100] pointer-events-none uppercase">{{ ub.badge?.name }}</div>
                </div>
              </div>
            </div>
          ...
          <div v-if="!isMyProfile && authStore.isAuthenticated" class="flex items-center gap-2">
            <template v-if="!displayedUser.isBanned">
              ...
              <button @click="handleMessageUser" class="p-2.5 bg-white dark:bg-gray-800 border border-slate-200 dark:border-white/10 rounded-full hover:bg-slate-50 dark:hover:bg-white/5 transition-all shadow-sm active:scale-95">
                <component :is="getBadgeComponent('mail')" class="w-5 h-5 text-slate-600 dark:text-gray-300" />
              </button>
            </template>
            
            <div class="relative" id="profile-options-container">
              <button @click="showOptionsMenu = !showOptionsMenu" class="p-2.5 bg-white dark:bg-gray-800 border border-slate-200 dark:border-white/10 rounded-full hover:bg-slate-50 dark:hover:bg-white/5 transition-all shadow-sm active:scale-95">
                <component :is="getBadgeComponent('more-horizontal')" class="w-5 h-5 text-slate-600 dark:text-gray-300" />
              </button>
              ...
              <Teleport to="body">
                <div v-if="showOptionsMenu" 
                     ...
                     :style="dropdownStyle">
                  <button v-if="!displayedUser.isBanned" @click="openUserReport" class="w-full text-left px-5 py-3 text-sm font-bold text-orange-600 hover:bg-orange-50 dark:hover:bg-orange-900/10 transition-colors flex items-center gap-3">
                    <component :is="getBadgeComponent('alert-triangle')" class="w-5 h-5" /> Şikayet Et
                  </button>
                  <button @click="handleBlockToggle" class="w-full text-left px-5 py-3 text-sm font-bold text-red-600 hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors flex items-center gap-3">
                    <component :is="getBadgeComponent('user-x')" class="w-5 h-5" /> {{ displayedUser?.isBlocked ? 'Engeli Kaldır' : 'Engelle' }}
                  </button>
                  <!-- ADMIN SECTION -->
                  <div v-if="authStore.user?.role === 'ADMIN'" class="mt-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                    <p class="px-5 py-1 text-[10px] font-black text-gray-400 uppercase tracking-widest">Yönetici Paneli</p>
                    <button @click="handleBanToggle" class="w-full text-left px-5 py-3 text-sm font-bold text-red-600 hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors flex items-center gap-3">
                      <component :is="getBadgeComponent('shield-off')" class="w-5 h-5" /> {{ displayedUser?.isBanned ? 'Yasağı Kaldır' : 'Yasakla' }}
                    </button>
                    <button @click="handleAdminDeleteUser" class="w-full text-left px-5 py-3 text-sm font-bold text-red-700 hover:bg-red-100 dark:hover:bg-red-900/20 transition-colors flex items-center gap-3">
                      <component :is="getBadgeComponent('trash-2')" class="w-5 h-5" /> Tamamen Sil
                    </button>
                    <button @click="showBadgeModal = true" class="w-full text-left px-5 py-3 text-sm font-bold text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-colors flex items-center gap-3">
                      <component :is="getBadgeComponent('award')" class="w-5 h-5" /> Rozet Yönetimi
                    </button>
                  </div>
                </div>
                <!-- Backdrop to close dropdown -->
                <div v-if="showOptionsMenu" @click="showOptionsMenu = false" class="fixed inset-0 z-[998] bg-transparent"></div>
              </Teleport>
            </div>
          </div>
        </div>
        ...
        <div v-if="displayedUser.isBanned" class="mt-8 mb-4">
          <div class="relative group overflow-hidden bg-gradient-to-br from-red-500/10 via-red-600/5 to-transparent border border-red-500/20 rounded-[2.5rem] p-8 text-center backdrop-blur-sm">
            <!-- Animated Red Pulse -->
            <div class="absolute -top-24 -right-24 w-48 h-48 bg-red-500/10 rounded-full blur-3xl animate-pulse"></div>
            <div class="absolute -bottom-24 -left-24 w-48 h-48 bg-red-600/10 rounded-full blur-3xl animate-pulse delay-700"></div>
            
            <div class="relative z-10 flex flex-col items-center gap-4">
              <div class="w-20 h-20 bg-red-600 rounded-full flex items-center justify-center shadow-2xl shadow-red-600/40 animate-bounce">
                <component :is="getBadgeComponent('shield-alert')" class="w-10 h-10 text-white" />
              </div>
              <h2 class="text-2xl sm:text-3xl font-black text-red-600 uppercase tracking-tighter">BU HESAP ASKIYA ALINDI</h2>
              <p class="max-w-md text-gray-600 dark:text-gray-400 text-sm sm:text-base font-medium leading-relaxed">
                Topluluk kurallarını ciddi şekilde ihlal ettiği tespit edildiği için bu hesaba erişim kalıcı olarak sınırlandırılmıştır. 
                <br><br>
                <span class="text-[10px] opacity-50 uppercase tracking-widest font-black">Güvenli bir kampüs ortamı için çalışıyoruz.</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <template v-else>
          <div class="mt-4 sm:mt-6 space-y-4 border-t border-slate-50 dark:border-white/5 pt-4 sm:pt-6 text-left">
            ...
            <div class="grid grid-cols-3 gap-1 sm:flex sm:gap-8 justify-items-center sm:justify-start items-center">
              ...
              <div @click="openFollowModal('followers')" class="flex flex-col sm:flex-row items-center gap-1 sm:gap-1.5 cursor-pointer group">
                <span class="text-[15px] sm:text-base font-black text-gray-900 dark:text-white group-hover:text-blue-500 transition-colors">{{ displayedUser._count?.followers || 0 }}</span>
                <span class="text-[11px] sm:text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-tight group-hover:text-blue-500 transition-colors">Takipçi</span>
              </div>
              <div @click="openFollowModal('following')" class="flex flex-col sm:flex-row items-center gap-1 sm:gap-1.5 cursor-pointer group">
                <span class="text-[15px] sm:text-base font-black text-gray-900 dark:text-white group-hover:text-blue-500 transition-colors">{{ displayedUser._count?.following || 0 }}</span>
                <span class="text-[11px] sm:text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-tight group-hover:text-blue-500 transition-colors">Takip</span>
              </div>
            </div>
          </div>
          ...
          <template v-if="canViewContent">
            ...
            <div v-else class="space-y-3 sm:space-y-4">
              <PostCard v-for="p in currentTabPosts" :key="p.id" :post="p" @delete="handleDeletePost" @showComments="handleShowComments" />
            </div>
          </template>
        </template>
      </div>
    ...
    <Teleport to="body">
      <transition name="fade">
        <div v-if="showGlobalReport" class="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md" @click.self="showGlobalReport = false">
          <div class="bg-white dark:bg-gray-900 w-full max-w-sm rounded-[2.5rem] shadow-2xl border border-gray-100 dark:border-white/10 flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            <div class="flex items-center justify-between p-6 border-b border-gray-100 dark:border-white/5">
              ...
              <button @click="showGlobalReport = false" class="p-2 hover:bg-slate-100 dark:hover:bg-gray-800 rounded-full text-slate-400">
                <component :is="getBadgeComponent('x')" class="w-5 h-5" />
              </button>
            </div>
            <div class="flex-1 overflow-y-auto p-4 space-y-1.5 max-h-[60vh] no-scrollbar">
              <div v-if="reportStep === 1">
                <button v-for="(subReasons, category) in reportCategories" :key="category" @click="selectReportCategory(category)" class="w-full text-left p-3.5 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 transition-all flex items-center justify-between group">
                  <span class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ category }}</span>
              ...
              <div v-else>
                ...
                <button v-for="sub in reportCategories[selectedReportCategory]" :key="sub" @click="submitReport(sub)" class="w-full text-left p-3.5 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 group">
        ...
        <div v-if="showBadgeModal" class="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md" @click.self="showBadgeModal = false">
          <div class="bg-white dark:bg-gray-900 w-full max-w-md rounded-[2.5rem] shadow-2xl border border-gray-100 dark:border-white/10 flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            <div class="p-6 border-b border-gray-100 dark:border-white/5 flex items-center justify-between">
              ...
              <button @click="showBadgeModal = false" class="p-2 hover:bg-slate-100 dark:hover:bg-gray-800 rounded-full text-slate-400">
                <component :is="getBadgeComponent('x')" class="w-5 h-5" />
              </button>
            </div>
            <div class="p-6 max-h-[60vh] overflow-y-auto no-scrollbar grid grid-cols-2 gap-3">
              <button v-for="badge in allBadges" :key="badge.id" @click="handleToggleBadge(badge.id)" :class="['relative p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 text-center group', hasBadge(badge.id) ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-transparent bg-slate-50 dark:bg-white/5 hover:border-slate-200 dark:hover:border-white/10']">
                <div class="w-10 h-10 rounded-full flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform" :style="{ backgroundColor: badge.color, color: 'white' }">
                ...
                <div v-if="hasBadge(badge.id)" class="absolute top-2 right-2 w-4 h-4 bg-blue-500 rounded-full flex items-center justify-center shadow-sm">
                  <component :is="getBadgeComponent('check')" class="w-2.5 h-2.5 text-white" />
                </div>
              </button>
            </div>
          </div>
...
const canViewContent = computed(() => (isMyProfile.value || !displayedUser.value?.isPrivate || isFollowing.value) && !displayedUser.value?.isBanned);
...
const handleBlockToggle = async () => {
  ...
  try {
    ...
    toast.success(displayedUser.value.isBlocked ? "Engellendi." : "Engel kaldırıldı.");
...
const handleBanToggle = async () => {
  ...
  try {
    ...
    toast.success(displayedUser.value.isBanned ? "Kullanıcı yasaklandı." : "Yasak kaldırıldı.");
    showOptionsMenu.value = false;
