  ...
  <div class="max-w-4xl mx-auto pb-20 text-left overflow-x-hidden">
    ...
    <template v-else-if="displayedUser">
      ...
      <div class="relative">
        ...
        <button v-if="isMyProfile" @click="showEditModal = true" class="absolute top-3 right-3 px-3 py-1.5 bg-black/50 hover:bg-black/70 text-white rounded-full text-[10px] sm:text-sm font-bold backdrop-blur-sm transition-all z-10">Profili Düzenle</button>
        <div class="absolute -bottom-10 sm:-bottom-16 left-4 sm:left-6">
          ...
          >
            ...
            <!-- Zoom Icon Hover (Web Only) -->
            <div v-if="displayedUser.avatarUrl" class="absolute inset-0 rounded-full bg-black/30 hidden sm:flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 backdrop-blur-[2px]">
              <component :is="getBadgeComponent('search-plus')" class="w-8 h-8 text-white" />
            </div>
          </div>
        </div>
      </div>
      ...
      <Teleport to="body">
        <transition name="fade">
          <div v-if="showAvatarZoom" class="fixed inset-0 z-[1000] flex items-center justify-center bg-black/95 backdrop-blur-2xl p-0" @click="showAvatarZoom = false">
            <button class="absolute top-6 right-6 p-2 text-white/50 hover:text-white z-[1010] active:scale-90 transition-all"><component :is="getBadgeComponent('x')" class="w-8 h-8 sm:w-10 sm:h-10" /></button>
            <div class="w-screen h-[100dvh] flex items-center justify-center p-4 sm:p-10" @click="showAvatarZoom = false">
              <img :src="getImageUrl(displayedUser.avatarUrl)" class="max-w-[95vw] max-h-[85dvh] w-auto h-auto object-contain rounded-2xl sm:rounded-3xl shadow-2xl animate-in zoom-in-90 duration-300" @click.stop />
              <p class="absolute bottom-8 left-1/2 -translate-x-1/2 text-white/30 text-[10px] font-black uppercase tracking-[0.3em] sm:hidden">Kapatmak için dokun</p>
            </div>
          </div>
        </transition>
      </Teleport>
      ...
      <div class="bg-white dark:bg-gray-900/40 rounded-b-xl border-x border-b border-slate-200 dark:border-primary-900/30 pt-12 sm:pt-20 px-4 sm:px-6 pb-6 shadow-sm overflow-hidden">
        <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
          <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-2 mb-1">
              <h1 class="text-lg sm:text-2xl font-black text-slate-900 dark:text-white truncate max-w-[200px] sm:max-w-full">
                {{ displayedUser.fullName || displayedUser.username }}
              </h1>
              <div v-if="(displayedUser.badges?.length || displayedUser.role === 'ADMIN')" class="flex flex-wrap gap-1 items-center">
                <div v-if="displayedUser.role === 'ADMIN'" class="p-1 rounded-full text-white bg-[#1E3A8A] shadow-sm"><component :is="getBadgeComponent('crown')" class="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5" /></div>
                <div v-for="ub in displayedUser.badges" :key="ub.badge.id" class="p-1 rounded-full border border-gray-100" :style="{ backgroundColor: ub.badge.color, color: getContrastColor(ub.badge.color) }"><component :is="getBadgeComponent(ub.badge.icon)" class="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5" /></div>
              </div>
            </div>
          ...
          <div v-if="!isMyProfile && authStore.isAuthenticated" class="flex items-center gap-2">
            <button @click="handleFollowToggle" :disabled="followLoading" class="flex-1 sm:flex-none px-5 py-2 sm:py-2.5 rounded-full font-black text-xs sm:text-sm transition-all" :class="[isFollowing ? 'bg-slate-100 dark:bg-gray-800 text-slate-900 dark:text-white' : 'bg-blue-600 text-white shadow-lg shadow-blue-500/20']">{{ isFollowing ? "Takipten Çık" : "Takip Et" }}</button>
            <button @click="router.push({ name: 'Messages', query: { userId: displayedUser.id } })" class="p-2 sm:p-2.5 rounded-full border border-slate-200 dark:border-gray-800 text-blue-600 transition-all active:scale-95"><component :is="getBadgeComponent('message-square')" class="w-4 h-4 sm:w-5 sm:h-5" /></button>
            <button @click="showOptionsMenu = !showOptionsMenu" class="p-2 sm:p-2.5 rounded-full border border-slate-200 dark:border-gray-800 transition-all active:scale-95"><component :is="getBadgeComponent('more-vertical')" class="w-4 h-4 sm:w-5 sm:h-5 text-gray-500" /></button>
          </div>
        </div>
        ...
        <div class="mt-4 sm:mt-6 space-y-4 border-t border-slate-50 dark:border-white/5 pt-4 sm:pt-6 text-left">
          ...
          <div class="grid grid-cols-3 gap-1 sm:flex sm:gap-8 justify-items-center sm:justify-start items-center">
            <div class="flex flex-col sm:flex-row items-center gap-1 sm:gap-1.5"><span class="text-xs sm:text-base font-black text-gray-900 dark:text-white">{{ displayedUser._count?.posts || 0 }}</span><span class="text-[9px] sm:text-sm font-medium text-gray-500 dark:text-gray-400">Paylaşım</span></div>
            <button @click="openFollowModal('followers')" class="flex flex-col sm:flex-row items-center gap-1 sm:gap-1.5 transition-all active:scale-95" :class="isMyProfile ? 'hover:opacity-70 cursor-pointer' : 'cursor-default'"><span class="text-xs sm:text-base font-black text-gray-900 dark:text-white">{{ displayedUser._count?.followers || 0 }}</span><span class="text-[9px] sm:text-sm font-medium text-gray-500 dark:text-gray-400">Takipçi</span></button>
            <button @click="openFollowModal('following')" class="flex flex-col sm:flex-row items-center gap-1 sm:gap-1.5 transition-all active:scale-95" :class="isMyProfile ? 'hover:opacity-70 cursor-pointer' : 'cursor-default'"><span class="text-xs sm:text-base font-black text-gray-900 dark:text-white">{{ displayedUser._count?.following || 0 }}</span><span class="text-[9px] sm:text-sm font-medium text-gray-500 dark:text-gray-400">Takip</span></button>
          </div>
        </div>
        ...
        <template v-if="canViewContent">
          ...
          <div v-else-if="currentTabPosts.length === 0" class="py-20 text-center text-slate-400 font-bold uppercase tracking-widest text-[10px]">Henüz içerik yok</div>
    ...
    <DeleteConfirmModal :is-open="confirmModal.show" :loading="confirmModal.loading" :title="confirmModal.title" :message="confirmModal.message" :confirm-text="confirmModal.confirmText" :variant="confirmModal.variant" @confirm="handleConfirmAction" @cancel="confirmModal.show = false" />
          ...
          <div class="bg-white dark:bg-gray-900 w-full max-w-sm rounded-[2.5rem] shadow-2xl border border-gray-100 dark:border-white/10 flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-1.5 no-scrollbar max-h-[60vh]">
              <div v-if="reportStep === 1">
                <button v-for="(cat, name) in reportCategories" :key="name" @click="selectReportCategory(name as string)" class="w-full text-left p-3.5 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 transition-all flex items-center justify-between group"><span class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ name }}</span><component :is="getBadgeComponent('chevron-right')" class="w-4 h-4 text-gray-300 group-hover:translate-x-1 transition-transform" /></button>
              </div>
              <div v-else class="space-y-1.5">
                <div class="px-4 py-2 text-[10px] font-black text-blue-500 uppercase tracking-widest">{{ selectedReportCategory }}</div>
                <button v-for="sub in reportCategories[selectedReportCategory]" :key="sub" @click="submitReport(sub)" :disabled="reportLoading" class="w-full text-left p-3.5 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 group"><span class="text-sm font-bold text-gray-700 dark:text-gray-300 group-hover:text-red-600">{{ sub }}</span></button>
              </div>
            </div>
          </div>
        </div>
...
const confirmModal = ref({ show: false, loading: false, title: "", message: "", confirmText: "", variant: 'info' as 'danger' | 'info', action: null as (() => Promise<void>) | null });
const openConfirm = (title: string, message: string, confirmText: string, variant: 'danger' | 'info', action: () => Promise<void>) => { confirmModal.value = { show: true, loading: false, title, message, confirmText, variant, action }; showOptionsMenu.value = false; };
const handleConfirmAction = async () => { if (!confirmModal.value.action) return; confirmModal.value.loading = true; try { await confirmModal.value.action(); confirmModal.value.show = false; } catch { toast.error("Hata!"); } finally { confirmModal.value.loading = false; } };

const followModal = ref({ isOpen: false, title: "", users: [] as any[] });
const openFollowModal = async (type: 'followers' | 'following') => { if (!isMyProfile.value) return; followModal.value.title = type === 'followers' ? 'Takipçilerim' : 'Takip Ettiklerim'; followModal.value.users = []; followModal.value.isOpen = true; try { const res = await apiClient.get(`/follow/${type}/${displayedUser.value.id}`); followModal.value.users = res.data.map((item: any) => type === 'followers' ? item.follower : item.following); } catch { toast.error("Hata!"); followModal.value.isOpen = false; } };
...
const reportCategories: Record<string, string[]> = { Nefret: ["Hakaretler", "Irkçı"], "Taciz": ["Hakaret"], "Şiddet": ["Tehdit"], Mahremiyet: ["Özel bilgi"], "Yasadışı": ["Satış"] };

const openUserReport = () => { reportType.value = "user"; reportTargetId.value = displayedUser.value.username; showGlobalReport.value = true; reportStep.value = 1; showOptionsMenu.value = false; };
const handleOpenPostReport = (postId: number) => { reportType.value = "post"; reportTargetId.value = postId; showGlobalReport.value = true; reportStep.value = 1; };
const selectReportCategory = (name: string) => { selectedReportCategory.value = name; reportStep.value = 2; };
const submitReport = async (sub: string) => { reportLoading.value = true; try { const p: any = { reason: selectedReportCategory.value, subReason: sub }; if (reportType.value === "user") p.reportedUsername = reportTargetId.value; else p.reportedPostId = reportTargetId.value; await apiClient.post("/users/report", p); toast.success("Bildirildi."); closeReport(); } catch { toast.error("Hata!"); } finally { reportLoading.value = false; } };
const closeReport = () => { showGlobalReport.value = false; reportStep.value = 1; };

const getImageUrl = (path: string) => { if (!path) return ""; if (path.startsWith("http")) return path; const apiUrl = import.meta.env.VITE_API_URL || "http://localhost:3000/api"; return `${apiUrl.endsWith("/api") ? apiUrl.slice(0, -4) : apiUrl}${path.startsWith("/") ? path : `/${path}`}`; };
const getContrastColor = (hex: string) => { if (!hex || hex === 'transparent') return 'white'; const r = parseInt(hex.slice(1, 3), 16), g = parseInt(hex.slice(3, 5), 16), b = parseInt(hex.slice(5, 7), 16); return (r * 299 + g * 587 + b * 114) / 1000 >= 128 ? "#0f172a" : "white"; };
...
const currentTabPosts = computed(() => { if (activeTab.value === "posts") return userPosts.value; if (activeTab.value === "reposts") return reposts.value; return likedPosts.value; });
...
const fetchProfile = async () => {
  ...
  } catch { error.value = "Hata!"; }
  finally { loading.value = false; postsLoading.value = false; }
};

const handleFollowToggle = async () => { if (!authStore.user) return; followLoading.value = true; try { const res = await followStore.toggleFollow(displayedUser.value.id); if (res.status === "FOLLOWING") { displayedUser.value.isFollowing = true; displayedUser.value._count.followers++; } else { displayedUser.value.isFollowing = false; displayedUser.value._count.followers--; } toast.success(res.message); } catch { toast.error("Hata!"); } finally { followLoading.value = false; } };
const handleSaveProfile = async (data: any) => { try { const res = await apiClient.patch(`/users/${displayedUser.value.id}`, data, { params: { currentUserId: authStore.user?.id }, headers: { "Content-Type": "multipart/form-data" } }); displayedUser.value = { ...displayedUser.value, ...res.data }; if (authStore.user?.id === Number(displayedUser.value.id)) authStore.updateUser(res.data); showEditModal.value = false; toast.success("Güncellendi!"); } catch { toast.error("Hata!"); } };
...
const handleDeletePost = async (postId: number) => { if (!confirm("Silinsin mi?")) return; try { await postsStore.deletePost(postId); fetchProfile(); } catch {} };
