...
<template>
  <div class="max-w-4xl mx-auto pb-20 text-left overflow-x-hidden">
    ...
    <template v-else-if="displayedUser">
      ...
      <div class="relative">
        <div
          class="h-32 sm:h-48 bg-gradient-to-r from-blue-500 to-indigo-600 dark:from-primary-700 dark:to-primary-800 rounded-t-xl overflow-hidden shadow-lg"
        >
          <img
        ...
        <div class="absolute -bottom-10 sm:-bottom-16 left-4 sm:left-6">
          ...
          >
            <img
              ...
              alt="Avatar"
              class="w-20 h-20 sm:w-32 sm:h-32 rounded-full border-4 border-white dark:border-gray-950 object-cover"
            />
            <div
              v-else
            ...
            <div
              v-if="displayedUser.avatarUrl"
              class="absolute inset-0 rounded-full bg-black/30 items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 backdrop-blur-[2px] hidden sm:flex"
            >
              <component
                :is="getBadgeComponent('search-plus')"
      ...
      <Teleport to="body">
        <transition name="fade">
          <div
            v-if="showAvatarZoom"
            class="fixed inset-0 z-[1000] flex items-center justify-center bg-black/95 backdrop-blur-2xl p-0"
            @click="showAvatarZoom = false"
          >
            <button
              class="absolute top-6 right-6 p-2 text-white/50 hover:text-white z-[1010] active:scale-90"
            >
              <component
                :is="getBadgeComponent('x')"
                class="w-8 h-8 sm:w-10 sm:h-10"
              />
            </button>
            <div
              class="w-screen h-[100dvh] flex items-center justify-center p-4 sm:p-10"
              @click="showAvatarZoom = false"
            >
              <img
                :src="getImageUrl(displayedUser.avatarUrl)"
                class="max-w-[95vw] max-h-[85dvh] w-auto h-auto object-contain rounded-2xl sm:rounded-3xl shadow-2xl animate-in zoom-in-90 duration-300"
                @click.stop
              />
              <p
                class="absolute bottom-8 left-1/2 -translate-x-1/2 text-white/30 text-[10px] font-black uppercase tracking-[0.3em] sm:hidden"
              >
                Kapatmak için dokun
              </p>
            </div>
      ...
      <div
        class="bg-white dark:bg-gray-900/40 rounded-b-xl border-x border-b border-slate-200 dark:border-primary-900/30 pt-12 sm:pt-20 px-4 sm:px-6 pb-6 shadow-sm overflow-hidden"
      >
        <div
        ...
        >
          <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-2 mb-1">
              ...
              <div
                v-if="
                  displayedUser.badges?.length || displayedUser.role === 'ADMIN'
                "
                class="flex flex-wrap gap-1 items-center"
              >
                <div
                  v-if="displayedUser.role === 'ADMIN'"
                  class="p-1 rounded-full text-white bg-[#1E3A8A] shadow-sm"
                >
                  <component
                ...
                <div
                  ...
                  :key="ub.badge.id"
                  class="p-1 rounded-full border border-gray-100"
                  :style="{
          ...
          >
            ...
            <button
              @click="
                router.push({
                  name: 'Messages',
                  query: { userId: displayedUser.id },
                })
              "
              class="p-2 sm:p-2.5 rounded-full border border-slate-200 dark:border-gray-800 text-blue-600 transition-all active:scale-95"
            >
              <component
            ...
            <div class="relative inline-block">
              <button
                ...
                class="p-2 sm:p-2.5 rounded-full border border-slate-200 dark:border-gray-800 transition-all active:scale-95"
              >
                <component
              ...
              <!-- DROPDOWN MENU (Mobil: Fixed, Web: Absolute) -->
              <div
                v-if="showOptionsMenu"
                class="fixed inset-0 z-[200] sm:absolute sm:inset-auto sm:right-0 sm:top-full sm:mt-2"
                @click.stop="showOptionsMenu = false"
              >
                <!-- Mobil Overlay (Transparan Tıklama Alanı) -->
                <div
                  class="absolute inset-0 bg-black/5 backdrop-blur-[2px] sm:hidden"
                ></div>

                <div
                  class="absolute right-4 top-20 w-56 bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-[2rem] shadow-2xl z-[210] py-2 animate-in fade-in zoom-in-95 duration-200 overflow-hidden sm:relative sm:right-0 sm:top-0 sm:mt-0 sm:w-56"
                  @click.stop
                >
                  <button
        ...
        >
          ...
          >
            ...
            <button
              ...
              class="flex flex-col sm:flex-row items-center gap-1 sm:gap-1.5 transition-all active:scale-95"
              :class="
                isMyProfile
              ...
              >
                <div
                  ...
                  :style="{
                    ...
                    border:
                      badge.color === '#FFFFFF' ? '1px solid #e2e8f0' : 'none',
                  }"
                >
                  <component
...
const openBlockConfirm = () => {
  ...
  openConfirm(
    ...
    isBlocking
      ? "Bu kullanıcıyla tüm takipleşme bağın kopacak."
      : "Bu kullanıcının engeli kaldırılacak.",
    isBlocking ? "ENGELLE" : "ENGELİ KALDIR",
...
const handleAdminBanToggle = () => {
  ...
  openConfirm(
    ...
    isBanning
      ? "Bu kullanıcı artık sisteme giriş yapamayacak."
      : "Bu kullanıcının yasağı kaldırılacak.",
    isBanning ? "YASAKLA" : "YASAĞI KALDIR",
    "danger",
    async () => {
...
const handleAdminDeleteUser = () => {
  openConfirm(
    "KALICI OLARAK SİL?",
    "Bu kullanıcı ve tüm verileri TAMAMEN silinecek. Geri dönüşü yok!",
    "SİL GİTSİN",
    "danger",
    async () => {
...
const handleToggleBadge = async (badgeId: number) => {
  try {
    ...
    if (res.data.assigned) {
      ...
      displayedUser.value.badges = displayedUser.value.badges.filter(
        (ub: any) => ub.badgeId !== badgeId,
      );
    }
...
const openFollowModal = async (type: "followers" | "following") => {
  ...
  followModal.value.title =
    type === "followers" ? "Takipçilerim" : "Takip Ettiklerim";
  followModal.value.isOpen = true;
  try {
    const res = await apiClient.get(
      `/follow/${type}/${displayedUser.value.id}`,
    );
    followModal.value.users = res.data.map((item: any) =>
...
const reportCategories: Record<string, string[]> = {
  Nefret: ["Hakaretler", "Irkçı"],
  Taciz: ["Hakaret", "Taciz"],
  Şiddet: ["Tehdit", "Zarar Verme"],
  Mahremiyet: ["Özel bilgi paylaşımı"],
  Yasadışı: ["Satış", "Suç teşkil eden içerik"],
};
...
const submitReport = async (sub: string) => {
  ...
  try {
    const p: any = { reason: selectedReportCategory.value, subReason: sub };
    if (reportType.value === "user") p.reportedUsername = reportTargetId.value;
    else p.reportedPostId = reportTargetId.value;
    await apiClient.post("/users/report", p);
    toast.success("Bildirildi.");
    closeReport();
  } finally {
...
const getContrastColor = (hex: string) => {
  ...
  return (r * 299 + g * 587 + b * 114) / 1000 >= 128 ? "#0f172a" : "white";
};
...
const canViewContent = computed(
  () =>
    isMyProfile.value || !displayedUser.value?.isPrivate || isFollowing.value,
);
  ...
  const update = (list: any[]) => {
    list.forEach((p) => {
      ...
      if (t.id === postId) {
        if (type === "like") {
          t.isLiked = status;
          t._count.likes += status ? 1 : -1;
        }
        if (type === "repost") {
          t.isReposted = status;
          t._count.reposts += status ? 1 : -1;
        }
      }
    });
  };
  update(userPosts.value);
  update(reposts.value);
