        ...
        <div class="space-y-3">
          ...
          >
            <div class="flex items-center gap-4">
              ...
              <span class="text-sm font-black text-gray-900 dark:text-white uppercase tracking-wider">Güncelleme Notları</span>
            </div>
...
"
      ...
      <div v-if="activeSection === 'updates'" class="space-y-8 pb-20 font-black">
        <div class="relative pl-8 space-y-12">
          <!-- Timeline Vertical Line -->
          <div class="absolute left-3 top-2 bottom-2 w-0.5 bg-gray-100 dark:bg-primary-900/20"></div>
          ...
          <div v-for="v in changelog" :key="v.version" class="relative">
            ...
            <div 
              ...
              :class="v.version.includes('1.4.1') ? 'bg-blue-500 scale-125' : 'bg-gray-300 dark:bg-gray-700'"
            ></div>

            <div class="space-y-4">
              <div class="flex items-center gap-3">
                <span class="px-3 py-1 bg-blue-600 text-white text-[10px] font-black rounded-full tracking-widest">{{ v.version }}</span>
                <span class="text-[10px] text-gray-400 uppercase tracking-wider">{{ v.date }}</span>
              </div>

              <div class="bg-gray-50/50 dark:bg-gray-900/40 rounded-3xl border border-gray-100 dark:border-primary-900/10 p-6 space-y-6">
                <div v-if="v.added?.length" class="space-y-3">
                  <h4 class="text-[10px] text-blue-500 uppercase tracking-[0.2em] font-black">Yenilikler</h4>
                  <ul class="space-y-2">
                    <li v-for="item in v.added" :key="item" class="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300 font-medium">
                ...
                <div v-if="v.fixed?.length" class="space-y-3 pt-2 border-t border-gray-100 dark:border-white/5">
                  <h4 class="text-[10px] text-emerald-500 uppercase tracking-[0.2em] font-black">Düzelttik</h4>
                  <ul class="space-y-2">
                    <li v-for="item in v.fixed" :key="item" class="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300 font-medium">
...
const changelog = [
  ...
  },
  {
    ...
    date: "20 Mart 2026",
    added: [
      "MAKUtalk Lansmanı: İlk büyük adım!"
    ]
  }
];
